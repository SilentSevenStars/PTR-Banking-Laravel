    ...
    <div class="max-w-5xl mx-auto space-y-8">
        ...
        <div class="bg-white p-6 rounded-xl shadow-md hover:shadow-lg transition">
            ...
            <form id="paymentForm" class="space-y-4">
                {{-- loan id provided by controller: return view('user.loan-view', ['id' => $id]); --}}
...
<script type="module">
    ...
    function loadLoanDetails() {
        $.post("{{ url('/loan/details') }}", {
            _token: "{{ csrf_token() }}",
            loan_id: LOAN_ID
        }, function(result) {
            try {
                const loanData = result && typeof result === 'object' ? result : JSON.parse(result);

                if (loanData && loanData.id) {
                    const amount = Number(loanData.amount);
                    monthly = Number(loanData.monthly_payment || 0);
                    const remaining = Number(loanData.remaining_balance || 0);
                    term = Number(loanData.term || 0);
                    monthsPaid = Number(loanData.months_paid || 0);

                    const progress = term > 0 ? Math.round((monthsPaid / term) * 100) : 0;
            ...
            $('#loanDetails').html(`<p class="text-red-600">Error fetching loan details.</p>`);
            $('#paymentForm').hide();
        });
    }
    ...
    function computePayment() {
        const selected = $('.monthChk:checked').map(function() {
    ...
    $('#paymentForm').on('submit', function(e) {
        ...
        $.post("{{ url('/loan/pay') }}", {
            ...
            try {
                const r = resp && typeof resp === 'object' ? resp : JSON.parse(resp);
                if (r.success) {
                    $("#successModalPayment").removeClass("hidden");
                    $("#successModalPayment h2").text(r.is_fully_paid ? "Loan Fully Paid!" : "Payment Successful");
                    $("#successModalPayment p").text(r.message);
                    loadLoanDetails();
                    loadLoanHistory();
                } else {
            ...
            alert("Request failed — check controller");
        });
    });

    function loadLoanHistory() {
        $.post("{{ url('/loan/history') }}", {
            _token: "{{ csrf_token() }}",
            loan_id: LOAN_ID
        }, function(res) {
            try {
                const history = res && typeof res === 'object' ? res : JSON.parse(res);

                if (Array.isArray(history) && history.length > 0) {
                    let html = '';
                    history.forEach((h, idx) => {
                        const installmentNo = h.installment_no ?? (idx + 1);
                        html += `
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-3 py-2 border">Month ${installmentNo}</td>
                                <td class="px-3 py-2 border">₱${parseFloat(h.amount).toFixed(2)}</td>
                                <td class="px-3 py-2 border">${h.paid_at ?? h.created_at ?? '---'}</td>
                            </tr>`;
                    });
